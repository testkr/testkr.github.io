...
directives.characterTable = function($rootScope, $timeout, $compile, $storage) {
    return {
		...
		link: function(scope, element, attrs) {
            ...
            // night toggle
            var nightToggle = $('<label class="night-toggle"><input type="checkbox"></input>야간모드</label>');
...
directives.addOrbOptions = function($timeout, $compile, MATCHER_IDS) {
    ...
    return {
        ...
        link: function(scope,element,attrs) {
            ...
            [ '힘', '기술', '속도', '마음', '지능', 'RCV', 'TND', 'BLOCK', 'EMPTY', 'BOMB', 'G' ].forEach(function(type) {
...
directives.addTags = function($stateParams, $rootScope) {
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            element.append($('<span class="tag flag">' + (flags.global ? '글로벌 캐릭터' : '일본판 캐릭터') + '</div>'));
            element.append($('<span class="tag flag">' +
                        (CharUtils.isFarmable(id) ? '파밍 가능' : '파밍 불가') + '</div>'));
            if (flags.rr) element.append($('<span class="tag flag">레어 동료</div>'));
            if (flags.lrr) element.append($('<span class="tag flag">씨크릿 스고페스</div>'));
            if (flags.promo) element.append($('<span class="tag flag">프로모션</div>'));
            if (flags.shop) element.append($('<span class="tag flag">레일리샵</div>'));
            if (flags.special) element.append($('<span class="tag flag">이벤트</div>'));
            if (CharUtils.checkFarmable(id, { '스토리': true }))
                element.append($('<span class="tag flag">스토리 드랍</div>'));

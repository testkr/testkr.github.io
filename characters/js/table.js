(function() {
    ...
    angular.module('optc') .run(function($rootScope, $timeout, $storage, MATCHER_IDS) {
        ...
        var tableFilter = function(settings, data, index) {
            ...
            // filter by server
            if (filters.server) {
                if (filters.server == '글로벌 캐릭터' && !flags.global) return false;
                if (filters.server !== '글로벌 캐릭터' && flags.global) return false;
            }
            // filter by rr pool
            if ((filters.rr === '레어 동료 제외' && flags.rr) || (filters.rr === '레어 동료' && !flags.rr)) return false;
            //filter by farmable Sockets
            if ((filters.socket === '비슬롯 캐릭터' && farmableSocket) || (filters.socket === '슬롯 캐릭터' && !farmableSocket)) return false;
        ...
        $timeout(function() {
            ...
            var types = { story: '스토리', fortnight: '스페셜 던전', raid: '강림', Coliseum: 'Coliseum', Treasure: '트레져 맵' };

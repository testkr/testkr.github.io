...
CharUtils.getIslandBonuses = function(y, day) {
    var result = [ ];
    if (day !== undefined) {
        if (day == CharUtils.getDayOfWeek(false)) result.push('글로벌:today'); 
        if (day == CharUtils.getDayOfWeek(true, true)) result.push('한/일:today'); 
    } else {
        ...
        if (global && drops['스토리'][y] && drops['스토리'][y].global) result.push('글로벌:' + global.type);
        if (japan) result.push('한/일:' + japan.type);
    }
    return result;
};

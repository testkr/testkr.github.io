(function() {
    ...
    var ORB_COLORS = { 힘: 'orangered', 속도: 'dodgerblue', 기술: 'lightgreen', 마음: 'gold', 지능: 'orchid', G: 'orange', S: 'orangered', RAINBOW: 'pink', MEAT: 'darkgoldenrod'};
    ...
    app.controller('ImageGeneratorCtrl', function($scope, $filter, $timeout) {
        ...
        $scope.$watch('numbers',function(numbers) {
            [ '힘', '속도', '기술', '마음', '지능' ].forEach(function(which, n) {
                ...
                type(context, { text: which+ ' 속성에 가하는 대미지 ', style: '10.5px "Open Sans"', x: 515, y: 30 + n * 70, align: 'center' });
            ...
            if ($scope.numbers.cost) {
                ...
                type(context, { text:'체력 : '+ currentHP + ' (' + percHP + '%)', x: baseX + 25, y: baseY });
                //RCV
			    awesome(context, { text: 'f0f5', x: baseX + 8, y: baseY + 25, align: 'center' });
                type(context, { text:'회복력 : ' + rcv, x: baseX + 25, y: baseY + 25 });
                //Cost
			    awesome(context, { text: 'f039', x: baseX + 8, y: baseY + 50, align: 'center' });
                type(context, { text:'코스트 : ' + cost, x: baseX + 25, y: baseY + 50 });
                //Heal per Turn
                if(healPerTurn){
        ...
        type(context, { text:'적 방어력 : ' + $filter('number')($scope.data.defense), x: baseX + 25, y: baseY + 20 });
        baseY += 25;
        ...
        type(context, { text: '적용된 필살기:', x: baseX + 25, y: baseY + 20 });
        var none = true;
        for (var i=0;i<6;++i) {

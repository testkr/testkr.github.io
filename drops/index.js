...
app.controller('MainCtrl',function($scope, $rootScope, $timeout, $controller) {
    ...
    if(searchQuery!=""){
        $scope.query = Utils.getRegex(decodeURI(searchQuery.replace(/%20스페셜 던전|%20강림/g,"").replace(/\?/,"\\?")));
    }
...
app.filter('smartSort',function($rootScope) {
    var getId = function(id) {
        id = Math.abs(id);
        return [ '힘', '기술', '속도', '마음', '지능' ].indexOf(window.units[id-1].type || '지능') * 1000 + id;
    };
...
function updateTimes(){
    ...
    document.getElementById("times").innerHTML = "글로벌: <b>"+moment().tz('Etc/GMT+8').format('H:mm:ss')+"</b> | 일본: <b>"+moment().tz('Asia/Tokyo').format('H:mm:ss')+"</b>";
    
    if(moment().tz('Asia/Tokyo').format('H')>12 && moment().tz('Asia/Tokyo').format('H')<23){
        document.getElementById("timesNote").innerHTML = "인게임 보너스 11시부터 23시까지<br><b>보너스타임 현재 진행중<b>";
    }else{
         document.getElementById("timesNote").innerHTML = "인게임 보너스 11시부터 23시까지<br><b>보너스타임 현재 미진행중<b>";
    }
    tt=refreshTimer();
}
window.onload = function() {
    //Also add the URI Search into the Search Bar
    document.getElementById("search").value = decodeURI(location.search.replace('?','').replace(/%20스페셜 던전|%20강림/g,"").replace(/\?/,"\\?"));
}
